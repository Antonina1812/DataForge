document.addEventListener('DOMContentLoaded', function () {
  ...
  function collectFieldsData() {
    return Array.from(fieldsContainer.querySelectorAll('.field-section')).map(sec => {
      const name        = sec.querySelector('.field-name').value.trim() || null;
      const type        = sec.querySelector('.field-type-select').value;
      const description = sec.querySelector('.description') .value.trim() || null;
      const constraints = getConstraints(sec, type);
      return { fieldName: name, fieldType: type, description, constraints };
    });
  }
